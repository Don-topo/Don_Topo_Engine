...
| H72 | Media | `src/Renderer/GpuResources.cpp:51` y `:125` | Vulkan pide una asignacion de memoria al driver POR RECURSO (16 sitios con `vkAllocateMemory` en crudo, **cero VMA**), mientras D3D12 suballoca con D3D12MA en sus 28 `CreateResource`. Son ~10 asignaciones por malla: vertices, indices y las tres texturas —que existen aunque la malla no tenga material, porque caen a marcadores de posicion— mas sus staging. | Medido: **697 ms en 9.996 llamadas** con 1000 mallas, el 27 % restante tras cerrar H71 (`vkCreateBuffer`/`vkCreateImage` en cambio son 13 ms: el coste es la ASIGNACION, no crear el objeto). Ademas hay un riesgo latente que no es de tiempo: `maxMemoryAllocationCount` suele valer 4096, asi que una escena de 1000 mallas distintas ya roza el techo y en otra GPU fallaria al cargar sin que falte VRAM. Arreglo = adoptar VMA; el camino por malla pasa por dos embudos (`createBuffer`/`createImage`), pero `VkDeviceMemory` aparece en 62 sitios de cabeceras y hay 57 `vkFreeMemory`. | L |
...
| **P21** · **ETAPAS 0 Y 1 CERRADAS** | Sombras de punto/foco de verdad: proyeccion en PERSPECTIVA desde la posicion de la luz, que es lo unico que hace que la sombra diverja y que su tamano dependa de la distancia. **Se descarto el planteamiento original** («un segundo sistema de sombras conviviendo con el actual»): las 6 caras de un cubemap comparten proyeccion y solo se diferencian por una rotacion, que ya la codifican la cara y la UV, asi que el shader no necesita 6 matrices. Cero cambios en el layout del UBO. **Etapa 0 (hecha)**: los shaders no tenian NINGUN `#include`, asi que `pbr.frag` y `fog.comp` mantenian cada uno su copia de la seleccion de cascada — el patron exacto de H65. Ahora hay `-I` + `DEPENDS` en el CMake y la logica vive en `shaders/shadow_lookup.glsl`. El filtrado NO se unifica (PCF 3x3 en la escena, un tap en la niebla): esa diferencia si era deliberada. **Etapa 1 (hecha)**: un FOCO proyecta con `perspectiveRH_ZO` desde su posicion, con el FOV de su cono, en la capa 0 del mapa de siempre. Cascadas y foco no coexisten nunca (una sola luz key, un solo tipo), asi que no hace falta ni recurso ni binding nuevos. **Etapa 2 (pendiente)**: luz de PUNTO con cubemap de 6 caras. Ya solo anade seleccion de cara. | `shaders/shadow_lookup.glsl` (nuevo), `pbr.frag`, `fog.comp`, `sandbox/CMakeLists.txt`, `UniformBufferObject.h` (`spotShadowMatrix`), `Passes/ShadowPass.*`, `Renderer.cpp`, `D3D12Renderer.cpp`. | **PropertiesPanel**, seccion Light: el aviso naranja deja de salir para un foco —su sombra ya es correcta— y solo queda para la luz de punto, remitiendo al foco como alternativa que si diverge. | Alto, y se cobro: el mapa del foco salia espejado en v SOLO en Vulkan, porque los dos backends absorben la convencion de Y en sitios distintos (Vulkan en la matriz, D3D12 en el viewport) y el helper compartido copio la forma de D3D12. El sintoma no era una sombra desplazada sino TITILEO, indistinguible de falta de bias. `spotShadowMatrix` lleva ahora `bool flipY`. | L |
