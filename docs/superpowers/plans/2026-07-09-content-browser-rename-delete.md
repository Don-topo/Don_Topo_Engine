...
- El hot-swap de textura "missing" (Task 5) cubre solo meshes estáticos (`go->staticRenderIndex`). Los meshes skinned no son alcanzables desde el Content Browser hoy (`loadMeshForSelected` solo carga `.fbx` estático vía `ModelLoader::load`, nunca `addSkinnedMesh`) — cubrir ese caso sería trabajo especulativo sin UI que lo dispare; queda fuera de alcance.
