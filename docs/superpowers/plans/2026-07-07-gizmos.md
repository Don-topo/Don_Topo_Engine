...
public:
    ...
    static void init(GpuDevice& gpu, VkRenderPass renderPass, VkFormat colorFormat);
    static void shutdown(GpuDevice& gpu);
    // frameIndex: mismo índice de frame-in-flight que usa Renderer (0..kFramesInFlight-1)
    // — cada frame en vuelo tiene su propio buffer para no pisar datos que la GPU
    // de un frame anterior todavía pueda estar leyendo (mismo motivo que
    // Renderer::m_uniformBuffers[MAX_FRAMES]).
    static void draw(VkCommandBuffer cmd, const glm::mat4& viewProj, int frameIndex);
    static void clear();
...
private:
    ...
    // Debe coincidir con Renderer::MAX_FRAMES (engine/include/DonTopo/Renderer.h).
...
{
    // Un buffer por frame-in-flight (igual que Renderer::m_uniformBuffers):
    // así el memcpy de un frame nunca pisa el buffer que la GPU de un frame
    // anterior todavía pueda estar leyendo.
