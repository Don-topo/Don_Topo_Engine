...
| AudioSource -> `bypassEffects` / `bypassListenerEffects` | **Ausente** | - (ningun bypass por voz) | Ahora SI hay efectos que puentear (P13), asi que la fila deja de ser teorica: un clip no puede escaparse del filtro de su bus. En Core API se resolveria enrutando esa voz a un ChannelGroup sin efectos; no se ha hecho porque anade un grupo por caso y nadie lo ha pedido. |
...
| AudioListener -> pausa global (`AudioListener.pause`) | **Ausente** | - | Un menu de pausa no puede callar el mundo de golpe. La referencia a `pauseBGM` de la version original de esta tabla ya no vale: esa API se retiro en H12. Con lo que hay hoy se emula bajando el bus Master a 0 desde Lua, que no es lo mismo (las voces siguen avanzando). El motor **tampoco** tiene pausa de Play Mode ni `timeScale`, asi que no hay nada con lo que sincronizarlo todavia. |
