...
- BGM — `loadBGM` ([AudioManager.cpp:102-116](../engine/src/Audio/AudioManager.cpp#L102), modo `2D|LOOP_NORMAL|CREATESTREAM|NONBLOCKING`), `playBGM`/`stopBGM`/`pauseBGM` ([AudioManager.cpp:238-274](../engine/src/Audio/AudioManager.cpp#L238)).
...
| AudioSource → `mute` | **Ausente** — se sustituye por volumen 0 | `AudioClipComponent.cpp:48-69` (solo volume) | Silenciar y restaurar exige que el script recuerde el volumen anterior. `Channel::setMute` existe en Core API. |
| AudioSource → `bypassEffects` / `bypassListenerEffects` | **Ausente** (no hay efectos que puentear) | — (ningún DSP en el módulo) | Sin impacto hoy; depende de que existan filtros. |
...
| AudioSource → curva de rolloff (`Logarithmic`/`Linear`/curva a mano) | **Ausente** — siempre la de fábrica de FMOD | `AudioManager.cpp:169` (solo `set3DMinMaxDistance`) | Sin control de la forma de la atenuación, solo del rango. Core API ofrece `FMOD_3D_LINEARROLLOFF`/`INVERSE`/`CUSTOMROLLOFF` en el `FMOD_MODE` **[verificar contra la API de FMOD]** el nombre exacto de las constantes. |
| AudioSource → `dopplerLevel` | **Ausente** | `AudioManager.cpp:47` (`vel = {0,0,0}` del listener), `AudioManager.cpp:214` (`v = {0,0,0}` de la fuente) | Un coche pasando de largo no cambia de tono. Requiere calcular velocidades por frame, que hoy no se calculan en ninguna parte. |
| AudioSource → `spread` | **Ausente** | — (sin `set3DSpread`) | Un sonido 3D colapsa a un punto; sin ensanchado estéreo de cerca. |
| AudioSource → paneo estéreo (`panStereo`) | **Ausente** | — (sin `setPan`) | No hay forma de colocar un sonido 2D a izquierda/derecha (típico de UI y diálogo). `Channel::setPan` existe en Core API. |
...
| AudioListener → pausa global (`AudioListener.pause`) | **Ausente** | — (solo `pauseBGM`, `AudioManager.cpp:269-274`) | Un menú de pausa no puede callar el mundo. El motor **tampoco tiene pausa de Play Mode ni `timeScale`** (grep: cero resultados), así que hoy no hay nada que sincronizar. |
...
| Snapshots del mixer y transiciones | **Ausente** | — | Sin "modo bajo el agua" ni ducking por estado. En Core API se emularía con transiciones de volumen/DSP a mano; **snapshots como concepto son de FMOD Studio**. |
| Filtros y efectos (lowpass, highpass, reverb, echo) | **Ausente** | — (ningún `FMOD::DSP` en el módulo) | Sin oclusión, sin ambientes, sin distorsión. Core API **sí** ofrece `System::createDSPByType` (`FMOD_DSP_TYPE_LOWPASS`, `HIGHPASS`, `ECHO`, `SFXREVERB`) — es implementable sin Studio. |
| Reverb Zones | **Ausente** | — | Sin cambio de ambiente al entrar en una cueva. Core API tiene `System::setReverbProperties` + `Channel::set3DOcclusion` y **hasta 4 instancias de reverb 3D** con `System::set3DReverbProperties` **[verificar contra la API de FMOD]** el límite exacto de instancias en la versión instalada. |
| One-shots sin GameObject (`PlayOneShot`, `PlayClipAtPoint`) | **Parcial** - `PlayOneShot` hecho 2026-08-27 (P5); `PlayClipAtPoint` (sin GameObject) sigue ausente, espera a P8 | `AudioManager.cpp:205` para la voz anterior del mismo id; `AudioClipComponent::play` es el único camino | **El hueco más visible en la práctica:** dos disparos seguidos del mismo clip no se solapan (el segundo corta al primero), y no hay forma de soltar un sonido en una posición sin crear un GameObject con componente. |
| Control de reproducción: `time`/`timeSamples` | **Ausente** | — (sin `getPosition`/`setPosition`) | No se puede arrancar un clip por la mitad ni saber por dónde va. `Channel::setPosition` existe en Core API. |
| Control de reproducción: `Pause`/`UnPause` de una fuente | **Ausente** para SFX (existe solo para BGM, que a su vez no se usa) | `AudioManager.cpp:269-274` (`pauseBGM`), sin equivalente por clip | Pausar exige `Stop()` y volver a empezar desde cero. |
| Control de reproducción: `isPlaying` consultable | **Ausente** en la API pública | `liveChannel` (`AudioManager.cpp:121-134`) lo sabe pero es `static` del .cpp | Un script no puede esperar a que termine un sonido. Es el binding más barato de todos: la lógica ya está escrita. |
