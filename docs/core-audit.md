...
`ABIERTO`; **H1 se cerró después** (ver P1), y su fila lo dice. El resto siguen
abiertas.
...
| P3 | **Alta** · ABIERTO | `src/Core/Scene.cpp:2737` | Cierra H14. Pasar una `PreloadedMeshCache` al `nodeFromJson` del clon, sembrada con la malla que el objeto origen **ya tiene en memoria** (`src->getMesh()`), igual que hoy se siembra la cache de `hasBones` cuatro líneas antes. El camino de copia profunda ya existe (`:1560-1566`). | **Medición, en Release** (Debug serializa el heap del CRT y mentiría): escena con un personaje skinned + un script que llama `Instantiate` una vez cada 60 frames. Arnés headless (`DonTopoRuntime` + escena por argv, tiempos a `game.log`). Se compara el ms del frame del spawn contra el de un frame sin spawn, antes y después. **Criterio**: el pico del frame del spawn deja de destacar sobre la media. Ojo al medir: con vsync todo sale a 16 ms — hay que mirar el tiempo de CPU del frame, no el de presentación. | M |
...
| 2 | **P3** (H14, carga de disco síncrona al `Instantiate`) | La otra Alta. Independiente de todo lo demás y con el camino de copia profunda ya escrito (`:1560-1566`): es cablear lo que ya existe. Desbloquea el spawn de personajes desde Lua, que hoy es un hitch por spawn |
...
> ejecución.
>
> **Trabajo hecho después, el mismo día**: **P1** (y con ella **H1**), la única
> Alta de corrección. Lo demás sigue como estaba. Cuando se cierre otra fila,
> que se anote aquí y **en su propia fila** — corregir solo el resumen deja la
> corrección invisible para quien lea la tabla.
