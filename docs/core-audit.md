...
**ACTUALIZACIÓN 2026-09-10/11: no queda ninguna fila abierta.** Las 12 que
seguían vivas se cerraron en esas dos jornadas, cada una con su test y con su
sabotaje —H2, H6, H7, H8, H9, H10, H11, H16, H17, H19, H20, H21 y P11—. Lo que
más se aprendió no fue de código: **la severidad escrita en una fila es la
estimación de quien la escribió, y falló en cuatro**. H9 no era un error blando
de GLFW, ASSERTA y aborta. H10 no era una consulta, era el cargador entero, con
la regla ya escrita y aplicada a un solo dispositivo de cuatro. H2 decía que
«cualquiera lanza» y 14 de los 66 `.value` no lanzan nunca. H21 decía que nadie
instanciaba `Engine` y los dos hosts la creaban. Y en H16 sobraba: el coste que
describía no lo paga ningún caller de hoy (se cambió por la semántica, no por el
reloj). **El test escrito ANTES del arreglo es lo que mide la fila; la fila solo
dice dónde mirar.**
...
| H8 | Baja · **CERRADO** (2026-09-10) | `src/Core/Scene.cpp:613-614, 642` | `animatorFromJson` no valida `entryState` ni `from`/`to` de las transiciones. `setEntryState` (`AnimatorComponent.cpp:61-66`) valida y **retorna sin hacer nada** si el índice está fuera de rango; `update` (`:478`) descarta una transición con `toState` inválido. | Un grafo guardado con `entryState: 3` y solo 2 estados (FBX reexportado, edición a mano) entra en el estado 0 sin decir nada, y una transición con `from` inválido queda muerta y muda. Todo lo demás en este fichero avisa de este tipo de anomalía: aquí no hay canal, aunque `animatorFromJson` ya recibe `warnings` y lo usa para los `readFloat`. | S | **Cerrado**: `animatorFromJson` valida contra los estados recién cargados. La transición con índice imposible se DESCARTA (no se acota: un índice inventado no se adivina, y apuntarla a un estado arbitrario sería peor que no tenerla) y `entryState` fuera de rango avisa sin cambiar el comportamiento — 0 sigue siendo lo único seguro. No avisa con el grafo VACÍO: ahí todo índice está fuera de rango y un animator recién creado no es un fichero corrupto. Tres tests, el tercero contra pasarse de guarda (grafo sano: 2 transiciones, entryState 1, cero avisos).
...
| H7 | Baja · **CERRADO** (2026-09-10) | `include/DonTopo/Core/Scene.h:65-71` vs `src/Core/Scene.cpp:3004-3044` | `Scene.h` declara `findCamera` "única fuente de verdad del invariante «como mucho una cámara por escena»". `fromJson` lo impone con `pruneExtraCameras` (`:3226`) y `cloneGameObject` con un descarte explícito (`:2754-2759`, con test). **`insertFromJson` no impone nada.** | Intenté alcanzar el estado de dos cámaras por esta vía y **no se puede hoy**: el stack de Undo es LIFO, así que para deshacer el borrado de la cámara A hay que deshacer antes el alta de la B. La fila se queda porque lo que protege el invariante es el orden de una estructura que vive fuera de `Scene`, mientras el header afirma que la fuente de verdad está dentro. Un cambio en el stack de Undo (saltar entradas, deshacer selectivo) lo rompería sin tocar `Scene`. | S | **Cerrado**: `insertFromJson` captura si ya hay cámara/oyente ANTES de insertar y descarta con aviso el del snapshot (gana el vivo, mismo criterio que su guarda de ids y que `pruneExtraCameras`). El AudioListener tenía el mismo agujero y entró en el mismo arreglo. Tres tests, incluido el que impide pasarse: sin cámara viva, deshacer la devuelve intacta.
| H6 | Baja · **CERRADO** (2026-09-10) | `src/Core/Scene.cpp:3146-3156` | `Scene::shutdown` pone a nulo los 4 colliders, el `AudioClip` y los scripts. **No toca** `Rigidbody`, `Animator`, `ReverbZone` ni `AudioListener` (`GameObject.h:318, 321-323`). | El `Rigidbody` guarda el actor de PhysX como `void* m_actor` (`Rigidbody.h:45-46`) y el actor lo libera `~Collider` (`Collider.h:73` + el comentario de `:155`): tras `shutdown()` ese puntero queda colgando. Hoy no se usa porque el único llamante (`fromJson:3208`) destruye acto seguido el árbol viejo con el move-assignment de `:3209`, así que ningún `Rigidbody` superviviente lo lee. Fila Baja por eso — pero la función se llama `shutdown`, es pública, y su comentario en `Scene.h:195` no dice ni qué limpia ni qué deja. Las zonas de reverb sobreviven igual y solo se recogen en el siguiente `syncReverbZones` (`:3143`), que en Edit Mode **no se llama** (`main.cpp:1023-1039`). | S | **Cerrado**: no añadiendo las 4 líneas que faltaban, sino quitando la lista. `shutdown` destruye el árbol (la raíz conserva id y nombre), así que corren los destructores de los 28 componentes y no hay nada que acordarse de enumerar — mismo cierre que el de `invalidateCaches`, que falló 4 veces enumerando. Contrato escrito en `Scene.h`, que era la otra mitad del hallazgo. Test con `weak_ptr`: prueba DESTRUCCIÓN, no que el GameObject deje de apuntar.
| H20 | Baja · **CERRADO** (2026-09-11) | `src/Core/Scene.cpp:3046` y `:3146` | `Scene::update(float, PhysicsManager&)` y `Scene::shutdown(PhysicsManager&, AudioManager&)` **no usan sus parámetros** — los tres van comentados en la firma. | No es cosmética: obliga a todo llamante a tener un `PhysicsManager` vivo para llamarlos. `engine/tests/camera_tests.cpp:8` documenta que los tests montan uno solo para satisfacer la firma, y con PhysX **solo cabe una `PxFoundation` por proceso**, así que ese `PhysicsManager` hay que compartirlo entre todos los tests del binario. Un parámetro muerto ha impuesto una restricción real al arnés de pruebas. | S | **Cerrado quitando los parámetros**: `update(float dt)` y `shutdown()`. `dt` SÍ se usaba (va a `updateAudioSpatial`), así que el muerto en `update` era solo el `PhysicsManager&`. Y la razón de quitarlos es la que da esta fila y no la estética: pedirlos no garantizaba nada —se podía pasar un manager ya apagado— mientras obligaba a todo llamante a tener uno vivo, y con una sola `PxFoundation` por proceso eso le imponía al arnés de tests compartir un `PhysicsManager` entre todos. **Rectifica lo que escribí al cerrar H6** unas horas antes, donde justifiqué los dos parámetros de `shutdown` como documentación de la dependencia: no lo eran. La obligación de orden (llamarla antes de destruir los managers) pasa a estar escrita en el header, que es donde sí se lee.
| H21 | Baja · **CERRADO** (2026-09-11) | `src/Core/Engine.cpp:5-7` y `include/DonTopo/Core/Engine.h:5-12` | La clase `Engine` es un constructor y un destructor **vacíos**. El header promete `throws std::runtime_error on init failure` (`Engine.h:7`) y no lanza nunca. | 23 LOC que no hacen nada y un comentario que describe un comportamiento inexistente. Nadie la instancia en `runtime/main.cpp` ni en `sandbox/src/main.cpp`. Borrarla o implementarla; lo que no puede quedarse es la promesa falsa en el header. | S | **Cerrado borrándola**, que era una de las dos salidas que ofrecía esta fila: no había nada que implementar — los dos hosts ya cablean todo explícitamente (Window, Input, backend, managers), así que la clase no tenía trabajo que reclamar. **Corrección a la fila**: decía que «nadie la instancia en runtime/main.cpp ni en sandbox/src/main.cpp» y era falso — los DOS la creaban (`DonTopo::Engine engine;`), como local muerta que nadie volvía a mirar; esas dos líneas y sus includes se van con ella. Fuera también de engine/CMakeLists.txt, así que hizo falta reconfigurar las dos builds.

### 2.4 Componentes: Animator, Input, Window

| ID | Severidad · Estado | fichero:línea | Qué pasa | Por qué importa / qué se encontró de verdad | Talla |
|---|---|---|---|---|---|
| H4 | Media · **CERRADO** | `src/Core/AnimatorComponent.cpp:52` y `:65` | `removeState` y `setEntryState` terminan en `reset()`, que borra `m_currentState` y **todos** los bool/trigger/int/float del usuario (`:280-296`). | `AnimatorComponent.h:158-163` documenta ese peligro palabra por palabra —y por eso existe `rebindClips`, que es `bindClips` sin el `reset()`— pero solo para el camino de las fuentes de animación. Los dos vecinos se quedaron con el `reset()`. Y son alcanzables en Play: `AnimatorPanel.cpp:435` y `:480` los llaman y `grep` confirma que **ese fichero no consulta `isPlaying` ni una vez**. Tocar el grafo a mitad de partida resetea la máquina de estados y los parámetros que el script Lua venía escribiendo. **Cerrado por P10.** | M |
| H5 | Media · **CERRADO** | `src/Core/Input.cpp:40, 187, 190, 206-211` | `takeActionDiagnostics()` **siempre devuelve una lista vacía**: `grep` sobre todo el repo dice que `s_actionDiagnostics` solo se declara (`Input.h:107`), se define (`Input.cpp:40`) y se vacía (`:209`). **Nadie escribe en ella.** | `Input.h:82-84` promete "avisos acumulados al cargar (bindings de mando ignorados)", y hay un consumidor real que los vuelca al Log (`ScriptBindings.cpp:501`). Los dos sitios que descartan un binding —código de eje fuera de rango (`:187`) y dispositivo desconocido (`:190`)— lo hacen con un `continue` mudo. Resultado: una acción que no dispara nunca porque su binding se descartó al cargar es indistinguible de una acción mal configurada, y el canal que existe para explicarlo estaba desconectado desde el primer día. **Cerrado por P9.** | S |
| H10 | Baja · **CERRADO** (2026-09-10) | `src/Core/Input.cpp:227` vs `:243-244` y `:260-261` | `isActionDown` resuelve un binding de ratón con `isMouseButtonDown(b.code)`, que consulta GLFW directamente **sin acotar el código** (`:89-92`). Sus dos hermanas sí lo acotan contra `GLFW_MOUSE_BUTTON_LAST` antes de indexar `s_mCurr`. | No hay acceso fuera de rango a memoria propia (GLFW valida y devuelve `RELEASE`), pero las tres funciones de la misma familia se comportan distinto ante el mismo `input_actions.json` corrupto: dos lo descartan y una se lo pasa a GLFW, que además emite un error de GLFW por consulta y por frame. | S | **Cerrado, y era más ancho de lo que decía esta fila**: el cargador ya tenía escrita la regla —«código fuera de rango: se descarta AQUÍ y no en cada consulta, y se NOMBRA»— pero aplicada solo a `padaxis`; `key`, `mouse` y `pad` entraban sin mirar. Validar los cuatro en el cargador hace que la cuarta consulta que alguien escriba herede la guarda. Y `isMouseButtonDown` pasa a acotar y a leer el snapshot del frame, como `isKeyDown`: antes preguntaba a GLFW en vivo, así que además podía discrepar de `isActionPressed`/`Released`, que sí leen `s_mCurr`. Probado en el CARGADOR a propósito: sin ventana de GLFW toda consulta da false y un test ahí pasaría igual de roto.
| H9 | Baja · **CERRADO** (2026-09-10) | `src/Core/Window.cpp:59-69` | `shouldClose()` llama a `glfwWindowShouldClose(m_window)` sin comprobar nulo, a diferencia de `show()` (`:54-57`) que sí lo hace. Y `shutdown()` llama a `glfwTerminate()` (`:63`), que apaga GLFW **para todo el proceso**. | Un `Window` sin `init` (o ya cerrado) pasa `nullptr` a GLFW, que lo trata como error de programación. Y `glfwTerminate` dentro del `shutdown` de una instancia convierte a `Window` en una clase que solo puede existir una vez por proceso, cosa que ni el header ni el nombre dicen. Hoy hay exactamente una en cada host, así que no muerde. | S | **Cerrado, y la severidad estaba SUBESTIMADA aquí**: `glfwWindowShouldClose(nullptr)` no devuelve un error blando, ASSERTA (`window.c: window != NULL`) — o sea aborta el proceso en Debug. Se comprobó escribiendo el test antes: abortaba. Ahora sin ventana devuelve true (los dos hosts giran en `while (!shouldClose())`: false ahí sería girar para siempre). El `glfwTerminate()` pasa a un contador de ventanas vivas, así que la última apaga la luz; hoy hay una por host, pero el nombre de la clase no dice «una por proceso». `init()` sobre una instancia con ventana ya no la pierde: hace shutdown antes. La guarda de `pollEvents` se queda por simetría y se DOCUMENTA como no load-bearing: quitarla no tumba nada (comprobado), solo evita un GLFW_NOT_INITIALIZED por frame.
| H11 | Baja · **CERRADO** (2026-09-10) | `src/Core/JobSystem.cpp:143-148` y `:103` | `cancel(id)` inserta en `m_cancelled` sin comprobar si el job existe, ya corrió o ya se descartó. El set solo se vacía en `shutdown()` (`:103`). | Un `cancel()` de un job que ya terminó deja su id dentro **para toda la vida del pool**. `AsyncAssetLoader.cpp:140` y `:380` cancelan cargas; en una sesión larga de editor con muchos Load Scene el set crece monótono. No es un bug de corrección (los ids no se reutilizan: `m_nextId` solo sube) y el coste por entrada es mínimo — es una fuga acotada por la duración de la sesión, dicha aquí para que no sorprenda. | S | **Cerrado**: el worker retira la marca también al TERMINAR el job, no solo al desencolarlo. Ese es el caso que se da de verdad —`AsyncAssetLoader` cancela cargas EN VUELO—; cancelar algo ya terminado no hace falta cubrirlo porque el loader detecta que el resultado está en el buzón y no llama a `cancel()`. Accesor `pendingCancellations()` para poder afirmarlo desde un test (y como diagnóstico: si solo sube, alguien cancela ids que este pool no vio).
...
| H16 | Baja · **CERRADO** (2026-09-10) | `include/DonTopo/Core/GameObject.h:278-283` | `traverse` toma `Fn fn` **por valor** y recursa con `c->traverse(fn)`, también por valor: una copia del functor por cada hijo y por cada nivel. | Con lambdas de captura pequeña es ruido; con las de `Scene::update` (`:3048`) y `collectLights` (`:2952`), que capturan por referencia, también. Se menciona porque el arreglo es de una línea (`Fn&& fn` + `std::forward`) y porque multiplica el coste de H12 por el número de nodos, no por el de recorridos. | S | **Cerrado, pero NO por lo que decía la fila**. Medido en /O2, 5000 nodos x 2000 recorridos: con las lambdas `[&]` de 8 bytes que usa TODO el repo, 25,3 ms por valor contra 24,3 por referencia — ruido, el coste que describe esta fila hoy no lo paga nadie. Solo con un functor de 264 bytes se ve: 62-69 ms contra 26-29, 2,4x. Se cambia a `Fn&&` por la SEMÁNTICA: por valor, un functor con estado propio perdía en silencio lo que sumaran los hijos (cada subárbol recibía su copia), y el test nuevo daba 0 en vez de 5. `Fn&&` y no `Fn&` porque casi todos los callers pasan la lambda en la propia llamada y un temporal no se engancha a una referencia lvalue.

### 2.6 Arquitectura

| ID | Severidad · Estado | fichero:línea | Qué pasa | Por qué importa / qué se encontró de verdad | Talla |
|---|---|---|---|---|---|
| H18 | Media · **DESCARTADO POR MEDICIÓN** | `src/Core/Scene.cpp:33-2633` vs `:2635-3241` | El namespace anónimo de serialización son **2.601 líneas, el 80,3 %** del fichero. La clase `Scene` son **607, el 18,7 %**. | El dato que importa no es el tamaño sino **quién arrastra las dependencias**: las 30 `#include` de `:1-32` (PhysicsManager, AudioManager, ModelLoader, AsyncAssetLoader, Cube/Sphere/Plane/Capsule, FileManager, ScriptComponent) las necesita casi toda la serialización y **casi ninguna la clase `Scene`**. Ver P6. **Medido el 2026-09-04 y descartado**, y de paso el criterio que esta fila proponía resultó ser malo: contar los `#include` de un `.cpp` no mide nada, porque **los includes de un `.cpp` no los ve nadie más**. Lo que sí se midió: `Scene.cpp` tarda **11,5 s** en compilar, sobre los **263 s** que cuesta tocar `Scene.h`. Es el **4 %**. El coste no está en el tamaño del `.cpp` sino en el alcance del `.h` — ver H19. | L |
| H19 | Media · **CERRADO** (2026-09-11) | `include/DonTopo/Core/GameObject.h:1-34, 312-340` | **33 `#include`** (28 de ellos de componentes concretos: UI, Physics, Audio, Core) y **28 miembros privados** (27 `shared_ptr` + 1 `vector`), más 12 campos públicos y 29 `has*()`. Lo incluyen **24** ficheros; a `Scene.h` (que lo incluye) lo incluyen otros **23**. | Dos consecuencias distintas, y solo una es de compilación. (a) Tocar cualquier componente de UI —`SliderComponent.h`, por ejemplo— recompila todo lo que ve `GameObject.h`. (b) La que no depende del build: `sizeof(GameObject)` son ~700 bytes de los que **27 punteros están a nulo en el nodo típico**, y los 9 recorridos por frame de H12 los pasean enteros por la cache. Un mapa de componentes arreglaría (b) y (a) a la vez, pero es una L que toca los 24 ficheros.<br><br>**Medido el 2026-09-04** (Debug, con las deps de ninja ya arregladas), y es peor de lo que la fila suponía:<br><br>| acción | TUs | tiempo |<br>|---|---|---|<br>| build limpio de los 87 TUs | 87 | **223 s** |<br>| `touch Core/Scene.h` | 18 | 263 s |<br>| `touch Core/GameObject.h` | 20 | 259 s |<br>| `touch UI/SliderComponent.h` (~100 líneas) | 23 | **266 s** |<br><br>**Tocar una cabecera cuesta MÁS que reconstruirlo todo desde cero.** Un fichero de UI de cien líneas arrastra 23 TUs, más que `Scene.h`, porque `GameObject.h` incluye los 28 componentes concretos y todo el editor incluye `GameObject.h`. Los 23 se llevan a los monstruos: `Renderer.cpp`, `ScriptBindings.cpp`, `PropertiesPanel.cpp`, `ViewportPanel.cpp`.<br><br>(El build limpio sale más rápido porque con 87 TUs ninja satura los cores; con 23 el reloj lo marcan las cadenas más largas y los enlaces finales.)<br><br>**Hay una vía más barata que el mapa de componentes**, sin tocar los 24 ficheros: `GameObject.h` puede **declarar por adelantado** los 28 tipos en vez de incluirlos, porque sus miembros son `std::shared_ptr<T>` (válidos con tipo incompleto) y su destructor **ya está fuera de línea** (`~GameObject();` declarado, `= default` en el `.cpp`), que es justo la condición que lo permite. Habría que mover a `GameObject.cpp` los **tres** sitios que sí exigen tipo completo: `isSkinned()` y `getSkinnedMesh()` (`GameObject.h:66-67`, por el `dynamic_cast`) y `anyCollider()` (`:97-104`, porque convertir `shared_ptr<BoxCollider>` a `shared_ptr<Collider>` necesita ver la herencia) y añadir sus propios `#include` a los consumidores que hoy viven de los transitivos — el compilador dice exactamente cuáles. **HECHO el 2026-09-04.** `GameObject.h` declara los 28 tipos en vez de incluirlos; `isSkinned`, `getSkinnedMesh` y `anyCollider` se mudaron al `.cpp`; 11 consumidores pasaron a incluir lo que usan.<br><br>**Y el resultado es más flojo de lo que la fila prometía**, medido con deps completas en las dos puntas (antes con el cambio en `stash`, después tras un rebuild total):<br><br>| | antes | después | |<br>|---|---|---|---|<br>| `touch UI/SliderComponent.h` | 34 TUs, 286 s | **25 TUs, 253 s** | −26 % TUs, **−12 % tiempo** |<br>| `touch Core/GameObject.h` | 28 TUs, 297 s | 28 TUs, 303 s | **sin cambio** |<br><br>`GameObject.h` no mejora, y tiene que ser así: lo sigue incluyendo todo el editor. Lo que se arregló es que tocar **un componente** ya no cascadea a través de él.<br><br>**Dónde está el resto del coste**: en `UI/UiWidgetSync.h`, que incluye los **14** componentes de UI y lo incluyen legítimamente `Renderer.cpp` y los paneles (es la maquinaria de sync). Eso ya no es Core y no entra en esta auditoría. Aviso para quien lo mire: al hacer este cambio le metí `UiWidgetSync.h` a `Scene.h` para resolver `UiCanvasBinding` y **deshice el arreglo entero sin enterarme** —la medición saltó a 33 TUs, peor que el baseline—. `Scene.h` declara ahora `Light` y `UiCanvasBinding` en vez de incluirlos: los dos solo aparecen como `std::vector<T>&` en parámetros, y eso vale con tipo incompleto.<br><br>**Lo que se lleva de premio aunque el reloj mejore poco**: los consumidores incluyen lo que usan en vez de vivir de una cadena transitiva de 28 headers. 25/25 en Debug y Release, sin un cambio de comportamiento. | L | **CERRADO el 2026-09-11, con lo que quedaba resuelto en dos frentes.** (1) **Verificado que el arreglo del 4-sep NO se ha deshecho**, que era el riesgo real de esta fila —se deshizo una vez sin que nadie se enterara—: medido con `ninja -n` (cuenta exacta de TUs, sin compilar), tocar `UI/SliderComponent.h` arrastra **26 TUs** hoy contra los 25 de entonces. Y ya no depende de que alguien se acuerde de medir: hay un test que LEE `GameObject.h` y falla si vuelve a aparecer un `#include` de `DonTopo/UI/`, `DonTopo/Physics/` o `DonTopo/Audio/` — comprobado devolviendo uno, y salta sin ni recompilar. También exige que las declaraciones adelantadas sigan ahí, para que no se pueda "arreglar" borrando las dos mitades. (2) **El mapa de componentes se DESCARTA**, no se aplaza: es una L que toca los 24 ficheros y su única consecuencia medida es la de H12 —los 9 recorridos por frame son el 1,4 % de un frame con 5.000 nodos, y una fusión ahorraría el 0,38 %—. El resto del coste de build vive en `UI/UiWidgetSync.h`, que no es Core. Se reabre si aparece un perfil que lo pague.
...
| P6 | Media · **DESCARTADA POR MEDICIÓN** (2026-09-04) | `src/Core/Scene.cpp` | Proponía mover el namespace anónimo (2.601 líneas) a su propio `.cpp`. **No se hace.** | **El criterio que traía esta fila no valía**, y eso se vio antes de mover una línea: "que `Scene.cpp` pierda 10 de sus 30 `#include`" es medible pero no significa nada — **los `#include` de un `.cpp` no salen de ese `.cpp`**. No desacoplan a nadie. Era un refactor de limpieza disfrazado de criterio binario, justo lo que el encargo de esta auditoría prohibía.<br><br>Así que se buscó el único beneficio que sí podría existir —**acortar el camino crítico del build en paralelo**— y se midió (Debug, con las dependencias de ninja ya arregladas en `b58bf36`; antes de eso ninguna medición de build era fiable):<br><br>`Scene.cpp` compila en **11,5 s** (15,9 s de tocarlo, menos 4,4 s de enlazarlo todo). Tocar `Scene.h` cuesta **263 s** y 18 TUs. Partir `Scene.cpp` en dos ahorraría, como mucho, **~6 s de 263**. <br><br>**No es rentable, y el motivo es otro que el que decía la fila**: el coste no está en que `Scene.cpp` sea grande, sino en a cuánta gente arrastra `Scene.h`. Ver H19, que ahora sí tiene un número. | L |
...
| P9 | Media · **HECHO** (2026-09-04) | `src/Core/Input.cpp` | Cierra H5. Los dos `continue` mudos ahora nombran la acción y el binding descartado. Además los avisos se limpian **al empezar cada carga**, no solo al leerlos: guardar el panel dos veces con un binding malo los duplicaba (mismo criterio que `Scene::m_warnings`), y el contrato del header se corrigió para decirlo. | **Criterio binario cumplido**: `testActionDiagnosticsReportDiscardedBindings` (`input_actions_tests.cpp`) escribe un fixture con un dispositivo inventado y un código de eje fuera de rango; `takeActionDiagnostics()` devuelve **2** avisos que nombran la acción, la segunda llamada devuelve 0, y la acción sobrevive con su binding bueno. **Visto en rojo primero** (devolvía 0). El fixture es el MISMO fichero que usa el editor, así que el test respalda el del usuario y lo devuelve. 2 sabotajes, uno por `continue`: cada uno tumba el test. | S |
| P10 | Media · **HECHO** (2026-09-04) | `src/Core/AnimatorComponent.cpp` | Cierra H4. `reset()` partido en dos, igual que `bindClips`/`rebindClips`: `resetPlayback()` mueve el playhead y corta el cross-fade **sin tocar los parámetros**, y `reset()` es eso más limpiarlos. `setEntryState` usa el primero. Y `removeState` va más allá: **reindexa** el playhead como ya hacía con las transiciones, en vez de mandarlo a la entrada — borrar otro estado no tiene por qué mover al usuario de sitio. | **Criterio binario cumplido**, con 4 tests y **5 sabotajes**, uno a uno. Y uno de los sabotajes destapó un test flojo: "mover siempre el playhead a la entrada" **no tumbaba nada**, porque en aquel test la entrada ERA el estado actual y las dos conductas daban el mismo resultado. Se añadió `test_remove_state_reindexes_playhead_not_resets_it`, que llega a un estado por una TRANSICIÓN con la entrada en otro sitio; con él, los 5 sabotajes caen cada uno por su test. 25/25 en Debug y Release. | S |
| P11 | Baja · **CERRADO en parte** (2026-09-11) | `src/Core/Scene.cpp:2765-2770` | Cierra parte de H17. Cortar el recorrido en la primera coincidencia y, en builds de depuración, seguir recorriendo para hacer `assert` de que no hay una segunda. | **Criterio binario**: con P1 aplicado, el `assert` nunca debe saltar; sin P1, el repro de H1 lo dispara. Sirve de red por si aparece otra fuente de ids repetidos. | S | Hecho el corte; el `assert` de unicidad **se RECHAZA a propósito**: el invariante se rompe en los tres caminos que INSERTAN nodos, y los tres ya lo guardan (`7fd087b`, `bb17088`, `6bf7ac7`), así que comprobarlo en los 192 sitios que LEEN es pagar en el lado equivocado. Además abortaría el editor ante un fichero que ahora se repara solo al cargar, y tumbaría los tests que construyen duplicados a propósito para fijar que gana el primero.
...
| ~~4~~ | **P9 + P10** (H5 y H4) — **HECHO 2026-09-04** | Las dos, con 7 sabotajes entre ambas. P10 además destapó un test propio que no distinguía lo que decía distinguir |
...
> dos mecanismos duplicados de aviso. Y **P6** (H18), medida y descartada: el
> criterio que traía —contar los `#include` de un `.cpp`— no medía nada, y el
> beneficio real que se buscó en su lugar salió al 4 %.
>
> **Lo que quedaba abierto —14 filas— se cerró el 2026-09-10/11.** Ninguna era
> un fallo reproducible cuando se escribió esta auditoría, y tres lo resultaron
> ser al ir a cerrarlas: H9 abortaba el proceso, H2 podía dejar un `.scene`
> incargable por un campo, y H7 dejaba dos cámaras en la escena tras un Undo.
> De H19 se hizo la vía barata (medida: −26 % de TUs al tocar un componente) y
> **el mapa de componentes se descartó** — su única consecuencia medida es el
> 1,4 % de un frame de H12, del que una fusión ahorraría el 0,38 %.
